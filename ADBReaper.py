...
def color_text(text, color):
    colors = {
        "red": Fore.LIGHTRED_EX,   # Brighter red
        "green": Fore.LIGHTGREEN_EX,  # Brighter green
        "yellow": Fore.LIGHTYELLOW_EX,  # Brighter yellow
        "blue": Fore.LIGHTBLUE_EX,  # Brighter blue
        "cyan": Fore.LIGHTCYAN_EX,  # Brighter cyan
        "magenta": Fore.LIGHTMAGENTA_EX,  # Brighter magenta
        "white": Fore.LIGHTWHITE_EX,  # Brighter white
        "bold": Style.BRIGHT,
        "dim": Style.DIM,
        "normal": Style.NORMAL
    }
    # Get the color code or empty string if the color doesn't exist
    color_code = colors.get(color, "")
    
    # Return the colored text with reset at the end
    return f"{Style.BRIGHT if color == 'bold' else ''}{color_code}{text}{Style.RESET_ALL}"
...
{color_text('    ██║  ██║██║     ██║     ██████╔╝███████╗╚██████╔╝', 'cyan')}
{color_text('    ╚═╝  ╚═╝╚═╝     ╚═╝     ╚═════╝ ╚══════╝ ╚═════╝ ', 'cyan')}
{color_text('          Remote ADB Control & Exploitation Tool', 'yellow')}
{color_text('       Developed by: ', 'red')} {color_text('Debojeet Bhowmick', 'green')}
{color_text('       Website: ', 'red')} {color_text('https://debojeet-bhowmick.netlify.app', 'blue')}
{color_text('       Github: ', 'red')} {color_text('https://github.com/D3b0j33t', 'blue')}
"""
    print(banner)
    logging.info("Banner Displayed")
...
def show_device_info():
    ...
    battery_level = adb_command(device, "shell dumpsys battery | findstr level") if os.name == 'nt' else adb_command(device, "shell dumpsys battery | grep level")
...
def check_battery_health():
    ...
    if not device:
        return
    battery_health = adb_command(device, "shell dumpsys battery | findstr health")
    charging_status = adb_command(device, "shell dumpsys battery | findstr status")
...
def check_adb():
    ...
    if "Android Debug Bridge" in result.stdout:
        ...
        print("[❌] ADB is not installed. Install it before running ADBReaper.")
        sys.exit(1)
...
def main():
    ...
    if not is_admin():
        print(COLOR_ERROR + "[!] This script requires Administrator privileges." + COLOR_DEFAULT)
    ...
    try:
        while True:
            ...
            if choice == '1':
                ...
                logging.info("Exiting ADBReaper. Goodbye!")
                break
            else:
        ...
        logging.error("Exiting ADBReaper. Goodbye!")
